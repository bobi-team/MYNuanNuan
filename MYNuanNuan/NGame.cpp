...
bool NGame::CaptureAndMatchMiniMap(const std::string& template_path, const std::string& result_path)
{
    std::string miniMapPath = "minimap.bmp";
    if (!CaptureMiniMap(miniMapPath)) {
        return false;
    }

    // 读取小地图截图和模板图像
    cv::Mat miniMapImage = cv::imread(miniMapPath, cv::IMREAD_COLOR);
    cv::Mat templateImage = cv::imread(template_path, cv::IMREAD_COLOR);

    if (miniMapImage.empty() || templateImage.empty()) {
        logger.error("无法加载小地图截图或模板图像");
        return false;
    }

    // 进行模板匹配
    cv::Mat result;
    cv::matchTemplate(miniMapImage, templateImage, result, cv::TM_CCOEFF_NORMED);
    ...
    // 绘制匹配结果
    cv::rectangle(miniMapImage, maxLoc, cv::Point(maxLoc.x + templateImage.cols, maxLoc.y + templateImage.rows), cv::Scalar(0, 255, 0), 2);
    cv::imwrite(result_path, miniMapImage);

    logger.info("模板匹配完成，结果保存为 " + result_path);
    return true;
}
