...
UINT CWHIniData::SetBool(LPCTSTR pszItem, LPCTSTR pszSubItem, bool bValue)
{
	int iValue = bValue ? 1 : 0;
	return SetInt(pszItem, pszSubItem, iValue);
}

UINT CWHIniData::SetInt(LPCTSTR pszItem, LPCTSTR pszSubItem, int iValue)
{
	CString strValue;
	strValue.Format(_T("%d"), iValue);
	return SetString(pszItem, pszSubItem, strValue);
}

UINT CWHIniData::SetString(LPCTSTR pszItem, LPCTSTR pszSubItem, LPCTSTR pszDefault)
{
	//效验状态
	ASSERT(m_szIniFile[0] != 0);

	//设置数值
	UINT uSetData = WritePrivateProfileString(pszItem, pszSubItem, pszDefault, m_szIniFile);
